...
begin
  ...
  if requested_chore_completion_id is not null then
    ...
    from public.allowance_entries allowance_entry
    where allowance_entry.chore_completion_id = requested_chore_completion_id
      and allowance_entry.household_member_id = request_row.household_member_id
    order by allowance_entry.created_at asc
    limit 1;
  end if;
