alter table public.allowance_request_entries
  add column if not exists request_kind text not null default 'credit';

alter table public.allowance_request_entries
  drop constraint if exists allowance_request_entries_request_kind_check;

alter table public.allowance_request_entries
  add constraint allowance_request_entries_request_kind_check
  check (request_kind in ('credit', 'debit'));

comment on table public.allowance_request_entries is
  'Pending child bank credits or debits that require household admin approval before an allowance ledger entry is created.';
